...
def resnet18(class_num=10, input_size=64):
    """
...
class TSSTLossNet(nn.Cell):
    def __init__(self, backbone):
        super(TSSTLossNet, self).__init__(auto_prefix=False)
        self._backbone = backbone
        self.mseloss = nn.MSELoss()
        self.entropyloss = nn.SoftmaxCrossEntropyWithLogits(sparse=True)

    def construct(self, x, has_object, object_class):
        y = self._backbone(x)
        ho = y[:, 0]
        oc = y[:, 1:]
        loss1 = self.mseloss(ho, has_object)
        loss2 = self.entropyloss(oc, object_class)
...
class TrainOneStepCell(nn.Cell):
    ...
    def __init__(self, network, optimizer, sens=1.0, reduce_flag=False, mean=True, degree=None):
        ...
        self.sens = Tensor((np.ones((ConfigResNet.win_batch_size,)) * sens).astype(np.float32))
    ...
    def construct(self, x, has_object, object_class):
        ...
        loss = self.network(x, has_object, object_class)
        grads = self.grad(self.network, weights)(x, has_object, object_class, self.sens)
        if self.reduce_flag:
